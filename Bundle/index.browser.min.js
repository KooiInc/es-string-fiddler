...
`).map(n=>n.replace(/\s|\/\/.*$/g,"").trim().replace(/(@s!)/g," ")).join(""),r)}function R(){let{isStringOrNumber:e,isObject:t,invalidate:r,replacement:n,replacer:c,replace:u}={isStringOrNumber:o=>[String,Number].find(s=>Object.getPrototypeOf(o??"")?.constructor===s),isObject:o=>Object.getPrototypeOf(o??"")?.constructor===Object,invalidate:(o,s)=>o??`{${s}}`,replacement:(o,s,l)=>!e(s[o])||!l&&`${s[o]}`.trim()===""?r(l,o):s[o]??r(l,o),replacer:(o,s)=>(...l)=>n(l.find(m=>m.key).key??"_",o,s),replace:(o,s,l)=>o.replace(/\{(?<key>[a-z_\d]+)}/gim,c(s,l))},L=(o,s=void 0,...l)=>l.flat().reduce((m,U)=>m.concat(t(U)?u(o,U,s):""),"");return(o,...s)=>L(o,void 0,...s)}}function z(){let{html:x,svg:F,tags:S}=tagsInfo,w={data:/data-[\-\w.\p{L}]/ui,validURL:/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|cid|xmpp):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i,whiteSpace:/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g,notAllowedValues:/javascript|injected|noreferrer|alert|DataURL/gi},v=$=>{let f=$?.nodeName.toLowerCase()||"none";return f==="#text"||!!S[f]};return C;function C($){let f={rawHTML:$.outerHTML,removed:{}};return $ instanceof HTMLElement&&[...$.childNodes].forEach(i=>{if(i?.children?.length&&C(i),i?.attributes){let j=i instanceof SVGElement?F:x;[...(i??{attributes:[]}).attributes].forEach(p=>{let h=p.name.trim().toLowerCase(),O=p.value.trim().toLowerCase().replace(w.whiteSpace,""),T=h==="href"?!w.validURL.test(O):w.notAllowedValues.test(O),g=h.startsWith("data")?!w.data.test(h):!!j[h];if(T||g){let E=p.value||"none";E+=E.length===60?"...":"",f.removed[`${p.name}`]=`attribute/property (-value) not allowed, removed. Value: ${E}`,i.removeAttribute(p.name)}})}if(!v(i)){let p=(i?.outerHTML||i?.textContent).trim()??"EMPTY";p+=p.length===60?"...":"",f.removed[`<${i.nodeName?.toLowerCase()}>`]=`not allowed, not rendered. Value: ${p}`,i.remove()}}),Object.keys(f.removed).length&&Object.entries(f.removed).forEach(([i,b])=>console.error(`${i}: ${b}`)),$}}
